...
public:
	long long minimalFatigue(vector <int> magicalGirlStrength, vector <int> enemyStrength, vector <long long> enemyCount) {
		...
		for (size_t i = 0; i < enemyStrength.size(); i++)
			v.push_back(make_pair(enemyStrength[i], i));
		sort(v.begin(), v.end());
		sort(magicalGirlStrength.begin(), magicalGirlStrength.end());
		if (magicalGirlStrength.back() < v.back().first)
			return -1LL;

		vector<long long> w(v.size());
		long long left = 0LL, right = accumulate(enemyCount.begin(), enemyCount.end(), 0LL);
		while (left < right) {
			long long mid = left + (right - left) / 2;
			for (size_t i = 0; i < w.size(); i++)
				w[i] = enemyCount[v[i].second];
			for (size_t i = 0; i < magicalGirlStrength.size(); i++) {
				long long remaining = mid;
				int sup_strength = magicalGirlStrength[i];
				for (size_t j = 0; j < w.size() && v[j].first <= sup_strength && remaining > 0; j++)
					if (w[j] > 0) {
						long long diff = min(remaining, w[j]);
						remaining -= diff;
						w[j] -= diff;
					}
			}
...
int main() {
	...
	int t0[] = {8,8,10,10,2};
			p0.assign(t0, t0 + sizeof(t0) / sizeof(t0[0]));
	int t1[] = {5,4,4,1,3,1,4,10,5,3,7,4,7,6,9,4,3};
			p1.assign(t1, t1 + sizeof(t1) / sizeof(t1[0]));
	long long t2[] = {20ll,20ll,20ll,20ll,20ll,20ll,20ll,20ll,20ll,20ll,20ll,20ll,20ll,20ll,20ll,20ll,20ll};
			p2.assign(t2, t2 + sizeof(t2) / sizeof(t2[0]));
